...
?>    
        ...
        <tr>
            <th>
                ...
                include("connect.php");
                $id=$_POST['supid'] ?? 0;
                $sup_acc=$_SESSION['supp'];
                $q1="select name, tax_number from vendor where account='$sup_acc'";
            ...
            <th>
                <label id='batch' name='batch'>BATCH: <?php echo $_SESSION['bno']; ?></label>
            </th>
        </tr>
        <tr>
            <th>
...
<?php
    ...
    $fromdate=$_SESSION['frmdate'];
    $todate=$_SESSION['todate']; 
    $sql = "select count(reference) as cnt from  postap a 
    join vendor b on a.accountlink=b.dclink where b.account= '$supplier'
    and format(a.txdate,'yyyy-MM-dd') between '$fromdate'
    and '$todate' and trcodeid=21";
    $params = array();	
    $stmt = sqlsrv_query($conn, $sql, $params) or die(print_r( sqlsrv_errors(), true));
    while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
        if (($row['cnt']!=0)) {?>
...
<?php } ?>
        <tr>
            ...
            <th>
                <label>BATCH: <?php echo $_SESSION['bno']; ?></label>
            </th>
        </tr>
        <tr>
            <th>
...
<?php
    ...
    $todate=$_SESSION['todate'];
    //get long inv no
    $long="select max(invoiceno) as long from _cplsupplierwvat a
    join postap b on a.invoiceid=b.autoidx
    join vendor c on c.dclink=b.accountlink
    where format((b.TxDate),'yyyy-MM-dd') 
    between '$fromdate' and 
    '$todate' and status='Yes'
    and c.account='$supplier'";
    $stmtone = sqlsrv_query($conn, $long) or die(print_r( sqlsrv_errors(), true));
    while( $rowone = sqlsrv_fetch_array( $stmtone, SQLSRV_FETCH_ASSOC) ) {
