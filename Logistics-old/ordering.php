...
where InvNum.OrderNum like '%$filtervalues%'
  and DocState in (1,3,4) and docflag = 1
group by 
Vendor.Account
,Vendor.Name
,InvNum.OrderDate
...
,convert(varchar,CPL_SageInternationalOrder.PIDate) as PIDate 
 ...
 and CPL_SageInternationalOrder.DocState in (1,3)
...
<td align="left"><div class="col"><input type="text" class="form-control" onChange= "updateOrderDate(this,'CPL_SageInternationalOrder.PIDate',<?php echo($LineID); ?>)" value = <?php echo $row['PIDate']; ?> > </div> </td>
...
<div id="myModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <?php 
      $code_query = "select distinct s.code from CPL_SagePoLine ce join invnum n on ce.poid=n.AutoIndex join _btblInvoiceLines bs on n.autoindex=bs.iInvoiceID
        join StkItem s on s.stocklink=bs.iStockCodeID
         where PONO  like '%$PONO%'";
      $code = sqlsrv_query($conn,$code_query);

  ?>
     <select name = "item_code" class="form-select" id = "item_code" >
       <?php
