<?php
        ...
        include("config.php");
          ...
          $query = $_POST['id'];
            ...
            where 
            _cplshipmentmaster.id=$query and cr.cost='AgencyFee'
            ...
            where ttr.id=$query and cr.cost='AgencyFee'
            ...
            select invoicelineid,code,cost,$amt*case when (cost)='AgencyFee' then nullif(sum(vol_weigh_amts+fobamt),0) else nullif(0,0) end as swift
            into #rates
            from #tmpamts 
            ...
            join _cplcostmaster cr on ts.costcode=cr.id
            where tr.id=cast($query as int)  and ts.active='True' and cr.cost='AgencyFee'";
