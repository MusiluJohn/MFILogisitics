...
if(isset($_POST["submit"])) {
    ...
    foreach ($_POST["users"] as $key => $value){
        ...
        isnull(bl.fQtyProcessed,0), (isnull(fUnitPriceInclForeign,0)-((fLineDiscount/100)*fUnitPriceInclForeign)),isnull(st.ufIIVolume,0)
        ,isnull(st.ufIIWeight,0),st.ucIIScheme,ce.Cost_Code,bl.iStockCodeID,'True',$id,Ordernum
        ,(isnull(bl.fQtyProcessed,0)*(isnull(fUnitPriceInclForeign,0)-((fLineDiscount/100)*fUnitPriceInclForeign))),(isnull(bl.fQtyProcessed,0)*isnull(st.ufIIWeight,0))
        ...
        join _cplcostmaster cr on ce.Cost_Code=cr.id  where bl.idinvoicelines=$id order by ilineid";
