    ...
    <?php
        ...
        include("config.php");
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
			...
			if ($stmt) {			 
			 echo "<select id='PO' name='PO' class='form-control select2' style='width:150px;'>";
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {				
					echo "<option  value=" .urlencode($row['ucIDPOrdShipmentNo']). "> " .$row["ucIDPOrdShipmentNo"]. "</option>";
				} 
