...
$qtyrecupdate="update _cplshipmentlines set rec_qty=(isnull(rec_qty,0))+$qtyrec[$i] where invoicelineid=$invlineid[$i]";
sqlsrv_query($conn, $qtyrecupdate) or die(print_r( sqlsrv_errors(), true));
}
...
where tm.id=cast($query as int) and rec_qty<>0
...
insert into _etblwhseibtlines (iwhseIBTId, istockid,cReference,iProjectID,bIsSerialItem,bIsLotItem,iLotID,fQtyIssued,fAdditionalcost,fQtyReceived,fQtyDamaged,fQtyVariance,fQtyOverDelivered,fNewReceiveCost,
fIssuedCost,iUnitsOfMeasureStockingID,iUnitsOfMeasureCategoryID,iUnitsOfMeasureID,fQtyRequired,fQtyApproved)

(select distinct @ibtid,max(stkcode) ,'IMPORT', 0,0,0,0,max(rec_qty),
(sum(isnull(cs.cost,0)))-max(case when cr.cost='VATonDuty' then (isnull(cs.cost,0)) else 0 end)-max(case when cr.cost='TT_SWIFT_Charges' then (isnull(cs.cost,0)) else 0 end) as addcost,
max(rec_qty),0,0,0,0,
(sum(isnull(cs.cost,0)))-max(case when cr.cost='VATonDuty' then (isnull(cs.cost,0)) else 0 end)-max(case when cr.cost='TT_SWIFT_Charges' then (isnull(cs.cost,0)) else 0 end) as addcost,
max(iuomstockingunitid) as unitid,
max(iuomstockingunitid) as unitid,
max(iuomstockingunitid) as unitid,
max(rec_qty),
max(rec_qty)
