<?php
        ...
        include("config.php");
          ...
          $query = $_POST['id'];
            ...
            where 
            _cplshipmentmaster.id=$query and cr.cost='Additional_Cost' and ServiceItem<>1
            ...
            where ttr.id=$query and cr.cost='Additional_Cost' and serviceitem<>1
            ...
            select invoicelineid,code,cost,$amt*case when (cost)='Additional_Cost' then nullif(sum(vol_weigh_amts+fobamt),0) else nullif(0,0) end as swift
            into #rates
            from #tmpamts 
            ...
            join _cplcostmaster cr on ts.costcode=cr.id join stkitem st on ts.stkcode=st.StockLink
            where tr.id=cast($query as int)  and ts.active='True' and cr.cost='Additional_Cost' and st.serviceitem<>1";
