...
<div id='body'style='margin-top:90px;'>
...
<a>Below are the lines of the selected shipment with the costs based on the scheme allocated to the item. If the scheme
   base is volume or weight the system will pick the highest between the two:</a>
...
<span class="glyphicon glyphicon-forward" style="margin-right:2px;"></span>PROCEED</button></td>
<?php
          include("config.php");
    ...
    <form id="myForm" method="POST" action="">
        ...
        $query = $_GET['SH'];
          $conn = sqlsrv_connect( $servername, $connectioninfo);
             $sql="select distinct (case when isnull(fexchrateusd,0)<>0 then  isnull(fexchrateusd,0) when isnull(fexchrateeur,0)<>0 then isnull(fexchrateeur,0) else isnull(rate,1) end) as rate from _cplshipmentlines cs join _cplshipmentmaster cr 
            on cs.shipment_no=cr.shipment_no join _cplshipment c on cr.shipment_no=c.cShipmentNo
            where cr.id=$query";    
             $stmt = sqlsrv_query($conn,$sql);     
                while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
                    echo "<a style='font-size:10px;margin-left:10px;font-weight:bold;'>Exchange Rate KES: </a> <input id='rate' name='rate' class='form-control' style='width:100px;' value=" .$row["rate"]. " ></td>";
                };
        sqlsrv_close($conn);
        ?>
        <script>
		  ...
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
            ...
            // $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
			 ...
			 $stmt = sqlsrv_query($conn,$sql2);		
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					echo "<a style='font-size:10px;margin-left:10px;'>Swift Charges KES: </a><input id=swift value=" .$row["cost"]. " name='swift' class='form-control' style='width:100px;length:50px;'/></td>";
				};
		sqlsrv_close($conn);
	    ?>
<!--calc swift--->
<script>
    ...
    $('#swift').change(function(){
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
             $sql="IF OBJECT_ID('tempdb..#tmpcost') IS NOT NULL DROP TABLE #tmpcost
             select distinct invoicelineid,code, st.cost,isnull(tm.shipment_no,'')as cshipmentno  into #tmpcost from _cplshipmentlines st join _cplshipmentmaster tm
             on st.shipment_no=tm.shipment_no join _cplcostmaster cr on st.costcode=cr.id
             join _cplshipment g on g.cShipmentNo=tm.shipment_no
             where tm.id=cast($query as int) and cr.cost='InsuranceAmount'";
             $sql1="insert into #tmpcost (code,cost) values('',0)";      
             $sql2 = "Select   round((case when max(b.finsurancechgsHome)<>0 then max(b.finsurancechgsHome) else sum(cost) end),2)  as cost
             from #tmpcost a join _cplShipment b on a.cShipmentNo=b.cShipmentNo";	
			// $params = array();
            // $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
             sqlsrv_query($conn,$sql);
             sqlsrv_query($conn,$sql1);	
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
            ...
            // $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
			 ...
			 $stmt = sqlsrv_query($conn,$sql2);		
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					echo "<a style='font-size:10px;margin-left:10px;'>Duty KES: </a><input id=duty value=" .$row["cost"]. " name='duty' class='form-control' style='width:100px;' disabled/></td>";
				};
		sqlsrv_close($conn);
	    ?>
<!--calc duty--->
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
            ...
            // $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
			 ...
			 $stmt = sqlsrv_query($conn,$sql2);		
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					echo "<a style='margin-left:9px;font-size:10px;'>Storage KES: </a><input id='storage' value=" .$row["cost"]. " name='storage' class='form-control' style='width:100px;'/></td>";
				};
		sqlsrv_close($conn);
	    ?>
<!--storage--->
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
            ...
            // $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
			 ...
			 $stmt = sqlsrv_query($conn,$sql2);		
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					echo "<a style='margin-left:9px;font-size:10px;'>Off Loading KES: </a><input id='offloading' value=" .$row["cost"]. " name='offloading' class='form-control' style='width:100px;'/></td>";
				};
		sqlsrv_close($conn);
	    ?>
<!--breakbulk--->
...
<?php
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);     
			 ...
			 $stmt = sqlsrv_query($conn,$sql);		
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					echo "<a style='margin-left:9px;font-size:10px;'>Trans Mode KES:</a><input id='mode' value='" .$row["mode"]. "' name='mode' class='form-control' style='width:100px;' disabled/></td>";
				};
		sqlsrv_close($conn);
	    ?>
</td>      
...
<script>
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
            ...
            // $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
			 ...
			 $stmt = sqlsrv_query($conn,$sql2);		
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					echo "<a style='font-size:10px;margin-left:10px;'>Entry/idf KES: </a><input id=entry value=" .$row["cost"]. " name='entry' class='form-control' style='width:100px;'/></td>";
				};
		sqlsrv_close($conn);
	    ?>
        <script>
    ...
    $('#entry').change(function(){
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
            ...
            // $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
			 ...
			 $stmt = sqlsrv_query($conn,$sql2);		
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					echo "<a style='font-size:10px;margin-left:10px;'>Customs Proc KES: </a> <input id='custproc' value=" .$row["cost"]. " name='custproc' class='form-control' style='width:100px;'/></td>";
				};
		sqlsrv_close($conn);
	    ?>
        </td>
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
             $sql="IF OBJECT_ID('tempdb..#tmpcost') IS NOT NULL DROP TABLE #tmpcost
             select distinct invoicelineid, code, st.cost,isnull(tm.shipment_no,'')as cshipmentno 
             into #tmpcost 
             ...
             $sql1="insert into #tmpcost (code,cost) values('',0)";  
			 $sql2 = "Select   round((case when max(b.fagencyfeesHOME)<>0 then max(b.fagencyfeesHOME) else sum(cost) end),2)  as cost
             from #tmpcost a join _cplShipment b on a.cShipmentNo=b.cShipmentNo";	
			// $params = array();
            // $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
             sqlsrv_query($conn,$sql);
...
<td>
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
             ...
             select distinct invoicelineid, code, st.cost into #tmpcost from _cplshipmentlines st join _cplshipmentmaster tm
             on st.shipment_no=tm.shipment_no join _cplcostmaster cr on st.costcode=cr.id
             where tm.id=cast($query as int) and cr.cost='Disbursement'";   
             $sql1="insert into #tmpcost (code,cost) values('',0)";  
			 $sql2 = "Select  round(sum(cost),2)  as cost
			 from #tmpcost";	
             sqlsrv_query($conn,$sql);
			 ...
			 $stmt = sqlsrv_query($conn,$sql2);		
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					echo "<a style='font-size:10px;margin-left:10px;'>Disbursement KES: </a> <input id='Disbursement' value=" .$row["cost"]. " name='Disbursement' class='form-control' style='width:100px;'/></td>";
				};
		sqlsrv_close($conn);
	    ?>
        </td>
        <script>
            ...
            $('#Disbursement').change(function(){
                $.ajax({
                    url: 'calc_disb.php',
                    type: 'post',
                    data: {id: <?php echo $_GET['SH']; ?>,disbursement: $('#Disbursement').val(),rate: $('#rate').val()},
                    success: function(result){
                        $('.toast').toast({
                            animation: false,
                            delay: 50000
        ...
        $query = $_GET['SH'];
          $conn = sqlsrv_connect( $servername, $connectioninfo);
             ...
             $stmt = sqlsrv_query($conn,$sql2);     
                while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
                    echo "<a style='font-size:10px;margin-left:10px;'>RailwayLevy KES: </a> <input id='railwaylevy' value=" .$row["cost"]. " name='railwaylevy' class='form-control' style='width:100px;' disabled /></td>";
                };
        sqlsrv_close($conn);
        ?>
        </td>
        ...
        <script>
            document.getElementById('railwaylevy').value = "<?php echo $_GET['railwaylevy'];?>";
        </script>
        <td>
        <?php
        include("config.php");
        $query = $_GET['SH'];
          $conn = sqlsrv_connect( $servername, $connectioninfo);
             ...
             $stmt = sqlsrv_query($conn,$sql2);     
                while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
                    echo "<a style='font-size:10px;margin-left:10px;'>GOK KES: </a> <input id='gok' value=" .$row["cost"]. " name='gok' class='form-control' style='width:100px;' disabled /></td>";
                };
        sqlsrv_close($conn);
        ?>
        </td>
    ...
    $('#otherawb').change(function(){
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
            ...
            // $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
			 ...
			 $stmt = sqlsrv_query($conn,$sql2);		
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					echo "<a style='font-size:10px;margin-left:10px;'>Excise Duty KES: </a><input id='exciseduty' value=" .$row["cost"]. " name='exciseduty' class='form-control' style='width:100px;' /></td>";
				};
		sqlsrv_close($conn);
	    ?>
        <script>
            ...
            $('#exciseduty').change(function(){
                $.ajax({
                    ...
                    type: 'post',
                    data: {id: <?php echo $_GET['SH']; ?>,exciseduty: $('#exciseduty').val(),rate: $('#rate').val()},
                    success: function(result){
                        $('.toast').toast({
                            animation: false,
                            delay: 50000
        ...
        $query = $_GET['SH'];
		  $conn = sqlsrv_connect( $servername, $connectioninfo);
			 ...
			 $stmt = sqlsrv_query($conn,$sql2);		
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					echo "<a style='font-size:10px;margin-left:10px;'>Additional Cost KES: </a> <input id='additionalcost' value=" .$row["cost"]. " name='additionalcost' class='form-control' style='width:100px;'/></td>";
				};
		sqlsrv_close($conn);
	    ?>
        <script>
...
<table id="cost_table" class="table table-sm table-bordered table-striped table-hover" style='font-size:10px;'>
        <thead >
            <tr>
                ...
                <th>Disbursement Cost KES</th>
                <th>Vat On Duty KES</th>
                <th>Additional Costs KES</th>
                <th>Landed Cost Total KES</th>
                <th>Landed Unit Cost KES</th>
                <th>Scheme</th>
                <th>Status</th>	
        ...
        include("config.php");
         ...
         $sql = "IF OBJECT_ID('tempdb..#shipment') IS NOT NULL DROP TABLE #shipment
                select invoicelineid as invoicelineid,max(po_no) as Po_No,max(cs.id) as id, (cs.shipment_no), code, max(description) as description, 
                max(case when cr.cost='Duty' then (ce.rate) else 0 end) as dutyperc,
                max(qty) as qty, max(amount) as amount,max(tot_amount_kes) as tot_amount_kes,max([volume]) as [volume],
                ...
                case when cr.cost='Disbursement' then max(cs.cost) else 0 end as disbcost,
                ...
                join _cplshipmentmaster b on cs.shipment_no=b.shipment_no
                where b.id=$query
                group by invoicelineid,code,cs.shipment_no,cr.cost,active  order by cs.shipment_no";
         $sql4="IF OBJECT_ID('tempdb..#totals') IS NOT NULL DROP TABLE #totals
            ...
            $sql1="select ROW_NUMBER() OVER( ORDER BY invoicelineid ASC) AS Row#, invoicelineid as invoicelineid,max(st.dutyperc) as dutyper,(case when max(calcduty)=1 then 'True' else 'False' end) as calcduty,max(factor) as factor,max(st.id) as id,isnull(max(code),'TOTAL') as code,(case when code is null then '' else max(po_no) end) as po_no,(case when code is null then '' else max(st.shipment_no)  end) as shipment_no, (case when code is null then '' else max(description) end) as description,
            (case when code is null then '' when max(qty)=0 then '' else max(qty) end) as qty, (case when code is null then '' else max(amount) end) as amount,(case when code is null then '' else max(tot_amount_kes) end) as tot_amount_kes,(case when code is null then '' else max([volume]) end) as [volume],
            (case when code is null then ''  when max([unit_weight])=0 then '' else max([unit_weight]) end) as [unit_weight], (case when max([volume])*167>max([unit_weight]) then max([volume])*167 else max([unit_weight]) end) as 'Highest',(case when code is null then ''  when max([weight])=0 then '' else max([weight]) end) as [weight], case when code is null then '' else (max(rate)) end as rate, round(sum(swift),2) as Swift,round(sum(insurance),2) as Insurance,round(sum(duty),2)  as Duty, round(sum(railway),2)  as Railway_Levy, round(sum(gok),2)  as GOK, round(sum(freight_for),2)  as Freight_For, round(sum(freight_ksh),2)  as Freight_Kshs, round(sum(entry),2)  as Entry, round(sum(penalty),2) as penalty, round(sum(handling),2)
            ...
            group by invoicelineid,code";
			 ...
			 //sqlsrv_query($conn,$duty,$params,$options) or die(print_r( sqlsrv_errors(), true));
             //sqlsrv_query($conn,$updateduty,$params,$options) or die(print_r( sqlsrv_errors(), true));
             //sqlsrv_query($conn,$exciseduty,$params,$options) or die(print_r( sqlsrv_errors(), true));
             //sqlsrv_query($conn,$updateexcise,$params,$options) or die(print_r( sqlsrv_errors(), true));
			 ...
			 if( $stmt === false) {
				...
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
                    ...
                    <td hidden><SELECT id='calcduty<?php echo $rows;?>' style="height:3%; width:70px;" class="form-select" name='calcduty<?php echo $rows;?>'> 
                        <option hidden value=1><?php echo $row["calcduty"] ;?></option>
                    ...
                    +$row["storage"]+$row["customsproc"]+$row["customsver"]+$row["doccharges"]+$row["breakbulk"]+$row["offloading"]+$row["transport"]+$row["othercharges"]  ;?></td>
		...
		<tfoot>
            <?php include("jquery.php"); ?>
                    ...
                    <th id="goktots"> <label id="goktotin"></a></th>
                    ...
                    <th id="totkestot"> <label id="unittotals"></a></th>
                    <th id="unitkestot"> </th>	
                    <th> </th>	
                    <th> </th>
                    </tr>
