<?php
        ...
        $qtys="--get the sumation
            ...
            where 
            _cplshipmentmaster.id=$query and cr.cost='TT_SWIFT_Charges' and ServiceItem<>1
        ...
        $amts="--get amts
            ...
            where ttr.id=$query and cr.cost='TT_SWIFT_Charges' and serviceitem<>1
        ...
        select invoicelineid,code,cost,$swift*case when (cost)='TT_SWIFT_Charges' then nullif(sum(vol_weigh_amts+fobamt),0) else nullif(0,0) end as swift
        into #rates
        from #tmpamts 
        ...
        update _cplshipmentlines set cost= swift,duty_modified_date=getdate()
            ...
            join _cplcostmaster cr on ts.costcode=cr.id join stkitem st on ts.stkcode=st.StockLink
            where tr.id=cast($query as int)  and ts.active='True' and cr.cost='TT_SWIFT_Charges' and st.serviceitem<>1";
