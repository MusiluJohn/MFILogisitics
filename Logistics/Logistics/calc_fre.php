<?php
        ...
        include("config.php");
          ...
          $query = $_POST['id'];
            ...
            where 
            _cplshipmentmaster.id=$query and cr.cost='FreightKsh' and ServiceItem<>1
            ...
            where ttr.id=$query and cr.cost='FreightKsh' and serviceitem<>1
            ...
            select invoicelineid,code,cost,$amt*case when (cost)='FreightKsh' then nullif(sum(vol_weigh_amts+fobamt),0) else nullif(0,0) end as swift
            into #rates
            from #tmpamts 
            ...
            join _cplcostmaster cr on ts.costcode=cr.id join stkitem st on ts.stkcode=st.StockLink
            where tr.id=cast($query as int)  and ts.active='True' and cr.cost='FreightKsh' and st.serviceitem<>1";
            $updatefor="update _cplshipmentlines set freight_for= swift/ts.rate
            from _cplshipmentlines ts join #rates ty on ts.invoicelineid=ty.invoicelineid
            and ts.costcode=ty.costcode 
            join _cplshipmentmaster tr on ts.shipment_no=tr.shipment_no
        ...
        sqlsrv_query($conn, $updatefor) or die(print_r( sqlsrv_errors(), true));
        ...
        *(max(ce.vat)/100) as vat
        ...
        select distinct invoicelineid, cs.code, (max(tot_amount_kes)/(max(amount)*max(qty)))
        as actualfactor
        into #actualfactor
        from stkitem st
        join _cplshipmentlines cs
        ...
        update _cplshipmentlines set correctfactor=b.actualfactor from _cplshipmentlines a join
