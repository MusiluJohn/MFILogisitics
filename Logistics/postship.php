<?php
   ...
   $checkshipno="select count(cShipmentNo)  as ship from _cplshipment where cshipmentNo='$shipno'";
             $stmt = sqlsrv_query($conn,$checkshipno);     
                while( $row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC) ) {

    if ($row['ship']==0) {
    
   $insertinshipment = "insert into _cplshipment (cshipmentno,cmode,detaport,detaoffice, fgrosswtkg,
   ...
   fotherchgsHOME,fexchrateUSD,fexchrateEUR,finsurancechgsHOME,fportchgsHOME,fagencyfeesHOME,
   fKEBSfeesHOME,ffreightchgsUSD,ffreightchgsEUR,cawbblno) 
   ...
   isnull($eurrate,0),isnull($inschgs,0),isnull($portchgs,0),isnull($agfees,0),isnull($kebsfees,0),isnull($freightusd,0),$freigheur,'$awbno')";
   sqlsrv_query($conn,$insertinshipment) or die(print_r( sqlsrv_errors(), true));
   echo('The shipment has been successfully created');
} else {
   echo('The shipment already exists. Kindly edit the shipment');
}
