...
cast(max(unit_amount_kes) as int)  unitcst,(max(unit_amount_kes)*max(qty))-(max(amount)*max(cs.rate)*max(qty)) as addcost,max(qty) as qty,max(iuomstockingunitid) as unitid
into #tmpcost
from stkitem st
