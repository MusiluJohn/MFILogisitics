<?php
        ...
        include("config.php");
          ...
          $query = $_POST['id'];
            $amt=$_POST['exciseduty'] ?? 0;
            $rate=$_POST['rate'] ?? 0;
            $qtys="--get the sumation
            IF OBJECT_ID('tempdb..#tmpship') IS NOT NULL DROP TABLE #tmpship
            select invoicelineid,(_cplshipmentlines.code),max(costcode) as cc , (amount) as fob,max(qty) as qt, (sum(volume)) as tot, sum(unit_weight*qty) as totweight,(sum(amount*qty))as amt 
            into #tmpship
            ...
            where 
            _cplshipmentmaster.id=$query and cr.cost='EXCISE_DUTY' and ServiceItem<>1
            ...
            (((amount*qty))/(select nullif(sum(amt),0) from #tmpship)) as fobamt
            into #tmpamts
            ...
            where ttr.id=$query and cr.cost='EXCISE_DUTY' and serviceitem<>1
            ...
            select invoicelineid,code,cost,$amt*case when (cost)='EXCISE_DUTY' then nullif(sum(fobamt),0) else nullif(0,0) end as swift
            into #rates
            from #tmpamts 
            ...
            join _cplcostmaster cr on ts.costcode=cr.id join stkitem st on ts.stkcode=st.StockLink
            where tr.id=cast($query as int)  and ts.active='True' and cr.cost='EXCISE_DUTY' and st.serviceitem<>1";
