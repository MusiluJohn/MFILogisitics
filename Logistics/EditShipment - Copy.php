...
<link rel="stylesheet" type="text/css" href="css/edit_shipment.css"/>
...
<?php
    //get shipment number
    include("config.php");
    $edit="select cShipmentNo from _cplshipment order by cShipmentNo";
...
<?php $_SESSION['edit_ship']=$_GET['shipnos']; ?>
...
<div id="dbcomments" style="height:400px;overflow:scroll">
<table>
    ...
    <?php
          include("config.php");
		  if (isset($_SESSION['edit_ship'])){
            $ship=$_SESSION['edit_ship'];
		  $results = array('error' => false, 'data' => '');
			 $sql = "select iCommentNo, cComment from _cplShipmentComments a
             join _cplShipment b on a.FKidShipment=b.idShipment
             where b.cShipmentNo='$ship' order by icommentno asc";
			 $params = array();
...
<tr><td style='width:100px;'><label hidden >Shipment ID: </label></td>
    ...
    <option><?php transport(); ?></option><option>Air</option><option>Sea</option><option>Courier</option>
    </select></td></tr>
    ...
    <tr><td style='width:100px;'><label style='margin-top:10px;'>Actual Arrival  @ Port: </label></td><td><input name='arrdate' id='arrdate' type='date' value='<?php arrivaldate(); ?>' class='form-control' style='margin-top:10px;'/></td></tr>
    ...
    <tr><td style='width:100px;'><label style='margin-top:10px;'>Custom Entry Date: </label></td><td><input name='customdate' id='customdate' value='<?php CustomEntryDate(); ?>' class='form-control' type='date' style='margin-top:10px;'/></td></tr>
    <tr><td style='width:100px;'><label style='margin-top:10px;'>Custom Pass Date: </label></td><td><input name='passdate' id='passdate' value='<?php CustompassDate(); ?>'class='form-control' type='date' style='margin-top:10px;'/></td></tr>
    ...
    <tr><td style='width:100px;'><label style='margin-top:10px;'>Clearing Agent: </label></td><td><select name='clagent' id='clagent' class='form-control' style='margin-top:10px;height:30px;'>
    <option><?php agent(); ?></option><option></option><option></option>
    </select></td></tr>
    <tr><td style='width:100px;'><label style='margin-top:10px;'>Status: </label></td><td><select name='status' id='status' class='form-control' style='margin-top:10px;height:30px;'>
    <option><?php status(); ?></option><option>IN TRANSIT</option><option>CLOSED</option>
    </select></td></tr>
...
<table  style="margin-left:60px;">
    ...
    <tr><td style='width:100px;'><label style='margin-top:10px;'>Payment Status: </label></td><td><select name='paystatus' id='paystatus' class='form-control' style='margin-top:10px;height:30px;'>
    <option><?php paymentstatus(); ?></option><option>UNPAID</option><option>PAID</option>
    </select></td></tr>
    ...
    <tr><td style='width:100px;'><label style='margin-top:10px;'>Freight Charges KES: </label></td><td><input name='portchgs' id='portchgs' class='form-control' type='number' value=<?php portcharges(); ?> style='margin-top:10px;'/></td></tr>
    ...
    <div class="form-check form-check-inline" style="margin-right:60px;margin-left:50px;">
        ...
        <?php
		  ...
		  $query = $_GET['shipnos'];
		  $results = array('error' => false, 'data' => '');
			 $sql = "select InvNumber from InvNum im
             join _etblUserHistLink ek on ek.TableID=im.autoindex 
			 join _rtblUserDict rt on ek.UserDictID=rt.idUserDict
             where ek.uservalue='$query' 
             and rt.cFieldName='ucIDPOrdShipmentNo'
             and docstate=4 and docflag=1";
			 $params = array();
			 $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
			 $stmt = sqlsrv_query($conn,$sql,$params,$options);		
			 if( $stmt === false) {
